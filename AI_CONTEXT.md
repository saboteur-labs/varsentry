...
You are assisting with a Node.js/TypeScript CLI tool called Varsentry. Apply the conventions below to every response. If a request would require violating the anti-patterns section, say so and ask for clarification rather than proceeding.

# Varsentry

This file provides guidance to local models when working with code in this repository.
